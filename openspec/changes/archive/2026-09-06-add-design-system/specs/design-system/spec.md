...
Los campos de formulario (`input`, `select`, `textarea`, `label`) SHALL compartir un estilo base consistente (padding, radio de borde, color de borde) usando los tokens definidos, SHALL mostrar un estado de error visualmente distinguible (borde/texto en `var(--color-danger)`) y un estado deshabilitado con contraste reducido (`design.md` § Goals).
